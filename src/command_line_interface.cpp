...
static std::string word_wrap(std::string const& input, const size_t width, std::string const& indent = "")
{
  ...
  while (i != input.end())
  {
    if (space_left - (i - word_start) < 1)   // line too long
    {
      ...
      out << std::endl << indent << variable_indent << utilities::substring(input, word_start, i);
    ...
    else if (*i == '\n')
    {
      if (word_start != i)
      {
        out << utilities::substring(input, word_start, i);
      }
  ...
  out << utilities::substring(input, word_start, input.end());
...
std::ostream& interface_description::option_descriptor::xml_page_description(std::ostream& s, const bool is_standard, unsigned int indentation) const
{
  ...
  // can be preserved.
  std::vector<std::string> lines = utilities::split(m_description, "\n");
...
{
  return "Copyright (c) " + copyright_period() + " " + copyright_owner() + ".\n"
...
std::string interface_description::man_page() const
{
  ...
  s << ".\\\" " << "Manual page for " << m_name << " version " << version() << "." << " .\\\"" << std::endl
